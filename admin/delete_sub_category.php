...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    $sql_get_cat = "SELECT category_id FROM sub_categories WHERE id = " . mysqli_real_escape_string($conn, $id);
    $result_get_cat = mysqli_query($conn, $sql_get_cat);
    $row_get_cat = mysqli_fetch_assoc($result_get_cat);
    $category_id = $row_get_cat['category_id'];

    $sql = "DELETE FROM sub_categories WHERE id = " . mysqli_real_escape_string($conn, $id);
