...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($name_err)){
        $sql = "UPDATE products SET category_id = " . mysqli_real_escape_string($conn, $category_id) . ", sub_category_id = " . ($sub_category_id ? mysqli_real_escape_string($conn, $sub_category_id) : "NULL") . ", brand_id = " . mysqli_real_escape_string($conn, $brand_id) . ", sub_brand_id = " . ($sub_brand_id ? mysqli_real_escape_string($conn, $sub_brand_id) : "NULL") . ", name = '" . mysqli_real_escape_string($conn, $name) . "', slug = '" . mysqli_real_escape_string($conn, $slug) . "', description = '" . mysqli_real_escape_string($conn, $description) . "', specifications = '" . mysqli_real_escape_string($conn, $specifications) . "', is_featured = " . mysqli_real_escape_string($conn, $is_featured) . ", is_new_arrival = " . mysqli_real_escape_string($conn, $is_new_arrival) . ", is_promotional = " . mysqli_real_escape_string($conn, $is_promotional) . " WHERE id = " . mysqli_real_escape_string($conn, $id);
            ...
            if(isset($_FILES['images'])){
                foreach($_FILES['images']['name'] as $key=>$val){
                    if(!empty($val)){
                        ...
                        $sql_image = "INSERT INTO product_images (product_id, image_path) VALUES (" . mysqli_real_escape_string($conn, $id) . ", '" . mysqli_real_escape_string($conn, $image_path) . "')";
    ...
    if(isset($_GET["id"]) && !empty(trim($_GET["id"]))){
        ...
        $sql = "SELECT * FROM products WHERE id = " . mysqli_real_escape_string($conn, $id);
                ...
                <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <select name="category_id" id="category_id" class="form-control">
                            ...
                            <option value="<?php echo $row_cat['id']; ?>" <?php echo ($category_id == $row_cat['id']) ? 'selected' : ''; ?>><?php echo $row_cat['name']; ?></option>
                            <?php } ?>
                        </select>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="is_featured" id="is_featured" value="1" <?php echo $is_featured ? 'checked' : ''; ?>>
                        <label class="form-check-label" for="is_featured">
                            Featured Product
                        </label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="is_new_arrival" id="is_new_arrival" value="1" <?php echo $is_new_arrival ? 'checked' : ''; ?>>
                        <label class="form-check-label" for="is_new_arrival">
                            New Arrival
                        </label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="is_promotional" id="is_promotional" value="1" <?php echo $is_promotional ? 'checked' : ''; ?>>
            ...
            if(category_id){
                $.ajax({
                    ...
                    success:function(html){
                        ...
                        $('#sub_category_id').val('<?php echo $sub_category_id; ?>');
                    }
                });
            }
            ...
            if(brand_id){
                $.ajax({
                    ...
                    success:function(html){
                        ...
                        $('#sub_brand_id').val('<?php echo $sub_brand_id; ?>');
                    }
                });
            }
